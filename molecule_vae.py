...
class ZincGrammarModel(object):
    ...
    def __init__(self, weights_file, latent_rep_size=56):
        ...
        self.vae.load(self._productions, weights_file, max_length=self.MAX_LEN,
    ...
    def encode(self, smiles):
        ...
        indices = [np.array([self._prod_map[prod]
                   for prod in entry], dtype=int)
                   for entry in productions_seq]
        one_hot = np.zeros((len(indices), self.MAX_LEN, self._n_chars),
                           dtype=np.float32)
    ...
    def _sample_using_masks(self, unmasked):
        ...
        for t in range(unmasked.shape[1]):
            ...
            rhs = [filter(f, self._productions[i].rhs())
...
class ZincCharacterModel(object):
    ...
    def __init__(self, weights_file, latent_rep_size=56):
        ...
        self.vae.load(self.charlist, weights_file, max_length=self.MAX_LEN,
                      latent_rep_size=latent_rep_size)

    def encode(self, smiles):
        """ Encode a list of smiles strings into the latent space """
        indices = [np.array([self._char_index[c] for c in entry], dtype=int)
                   for entry in smiles]
        one_hot = np.zeros((len(indices), self.MAX_LEN, len(self.charlist)),
                           dtype=np.float32)
        for i in range(len(indices)):
            num_productions = len(indices[i])
            one_hot[i][np.arange(num_productions), indices[i]] = 1.
    ...
    def decode(self, z):
        ...
        char_matrix = np.array(self.charlist)[np.array(sampled_chars,
                                                       dtype=int)]
