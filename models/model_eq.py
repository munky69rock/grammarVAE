...
class MoleculeVAE():
    ...
    def create(self,
               ...
               max_length=MAX_LEN,
               latent_rep_size=10,
               hypers={'hidden': 200, 'dense': 200, 'conv1': 2, 'conv2': 3,
                       'conv3': 4},
               weights_file=None):
    ...
    def _encoderMeanVar(self, x, latent_rep_size, max_length,
                        epsilon_std=0.01):
        h = Convolution1D(self.hypers['conv1'], self.hypers['conv1'],
                          activation='relu', name='conv_1')(x)
        h = BatchNormalization(name='batch_1')(h)
        h = Convolution1D(self.hypers['conv2'], self.hypers['conv2'],
                          activation='relu', name='conv_2')(h)
        h = BatchNormalization(name='batch_2')(h)
        h = Convolution1D(self.hypers['conv3'], self.hypers['conv3'],
                          activation='relu', name='conv_3')(h)
    ...
    def _buildEncoder(self, x, latent_rep_size, max_length, epsilon_std=0.01):
        ...
        def conditional(x_true, x_pred):
            ...
            # cast indices as ints
            ix2 = tf.cast(ix2, tf.int32)
            # get slices of masks_K with indices
            M2 = tf.gather_nd(masks_K, ix2)
            # reshape them
            M3 = tf.reshape(M2, [-1, MAX_LEN, DIM])
            # apply them to the exp-predictions
            P2 = tf.mul(K.exp(x_pred), M3)
            # normalize predictions
            P2 = tf.div(P2, K.sum(P2, axis=-1, keepdims=True))
            return P2
    ...
    def load(self, charset, weights_file, latent_rep_size=10,
             max_length=MAX_LEN, hypers={
                 'hidden': 200,
                 'dense': 200,
                 'conv1': 2,
                 'conv2': 3,
                 'conv3': 4}):
        self.create(charset, max_length=max_length, weights_file=weights_file,
                    latent_rep_size=latent_rep_size, hypers=hypers)
